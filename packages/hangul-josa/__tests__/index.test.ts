...
it("basic tests", () => {
  const player = "지현";
  const skill = "구르기";
  assert.equal(josa`${player}${는} ${skill}${을} 사용했다!`, "지현은 구르기를 사용했다!");

  const fruit = "파인애플";
  const apple = "사과";
  assert.equal(josa`${fruit}${와} ${apple}${이} 잘 썰려있다.`, "파인애플과 사과가 잘 썰려있다.");

  const dr_kim = "김 박사";
  const paper = "논문";
  assert.equal(josa`${dr_kim}${은} ${paper}${를} 제출했다.`, "김 박사는 논문을 제출했다.");
